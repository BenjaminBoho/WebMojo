...
var minvaltacc = 1.0 #加速度1.0m/s2
var minvalspeed = 0.3 #速度0.3mm/s
...
def trend_plot(prms):
    ...
    let results = trcom.get_trend(sensortype, sensorid, fromdt, todt)
    if len(results) == 0:
...
#メイン処理
fn main() raises:
    Python.add_to_path("/Users/rei/trcomWeb/IoTService/IoT/IoT/Brain/trcom.py")
    Python.add_to_path("/Users/rei/trcomWeb/IoTService/IoT/IoT/Brain/daily.py")
    Python.add_to_path("/Users/rei/trcomWeb/IoTService/IoT/IoT/Brain/rising_trend_alarm.py")
    Python.add_to_path("/Users/rei/trcomWeb/IoTService/IoT/IoT/Brain/suggest_threshold.py")
    let daily = Python.import_module("daily")
    let suggest_threshold = Python.import_module("suggest_threshold")
    let rising_trend_alarm = Python.import_module("rising_trend_alarm")
    let trcom = Python.import_module("trcom")
    let pd = Python.import_module("pandas")
    let np = Python.import_module("numpy")
    let plt = Python.import_module("matplotlib.pyplot")
    let mdates = Python.import_module("matplotlib.dates")
    let json = Python.import_module("json")
    let traceback = Python.import_module("traceback")
    let datetime = Python.import_module("datetime")
    let params = daily.initcommon()
