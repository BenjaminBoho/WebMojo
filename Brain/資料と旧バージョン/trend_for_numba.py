...
def trend_plot(prms):
    ...
    nolifeday = datetime.datetime.utcnow() + timezoneoffset - datetime.timedelta(days=nolifespancalcdays) #比較する日付
    ...
    #*****************************************************************野呂：UTCのずれを補正
    # #shiftの場合は時間でなくて行がずれるので上でデータの時間を変更
    #df["y"]=df["y"].shift(timezoneoffset) 
    #df.drop(df.index[range(timezoneoffset)],inplace=True)
    
    #df.to_csv("C:/Users/H4869/TR-BRAIN/df.csv")**********************野呂：確認用
    #results.to_csv("C:/Users/H4869/TR-BRAIN/results.csv")************野呂：確認用  
    ...
    plthistgraph(prms, "filtered_diff_hist" , df_filtered_diff)
    ...
    #まずはk-meanから
    #G-meanや混合正規分布からkを自動決定する方法に変更予定。
    
    # パラメータに変更 cluster_number=2 #クラスタ数　20200904変更＠野呂
    neinum = len(array_df_filtered)//cluster_number #いわゆるkの事　20200904変更＠野呂
    if (neinum <= 0):
    ...
    def calc_KNN_table(KNN_table):
        ...
        return
    calc_KNN_table(KNN_table)    
        
    #LOF
    knn_lookup_table = {}

    def calc_knn_lookup_table(knn_lookup_table):
        for i in range(array_df_filtered_diff.shape[0]):
            knn_lookup_table[tuple(array_df_filtered_diff[i])] = KNN_table[i] #各点の上位k個のデータ
        return
    calc_knn_lookup_table(knn_lookup_table)
    ...
    def calc_local_reach_density_lookup_table(local_reach_density_lookup_table):
        for i in range(array_df_filtered_diff.shape[0]):
        ...
        return
    calc_local_reach_density_lookup_table(local_reach_density_lookup_table)

    LOFs = []
    Local_Outlier_Factor_lookup_table = {}
    #Calculate the LoF for each point

    def calc_Local_Outlier_Factor_lookup_table(LOFs, Local_Outlier_Factor_lookup_table):
        for i in range(array_df_filtered_diff.shape[0]):
            pt = array_df_filtered_diff[i]
            knns = KNN_table[i] #上位k個のデータ
            lrd_knns = 0
        ...
        return
    #LOFが大きいほど外れ値
    calc_Local_Outlier_Factor_lookup_table(LOFs, Local_Outlier_Factor_lookup_table)

    #散布図とヒストグラムをプロットして外れ値を確認
    plt.figure()
    ax = plt.subplot()
    
    ax.scatter(array_df_filtered_diff,LOFs) #20200904修正＠野呂
    savefig(prms, "lofscat")    

    plt.figure()
    #ax = plt.hist(np.isfinite(LOFs), bins=100)
    ax = plt.hist(np.array(LOFs)[np.isfinite(LOFs)], bins=100) #histがエラーとなるため修正
    ...
    LOF_df_diff=LOF_df_diff.set_index(df_filtered.index[1:])
    ...
    #ハイパーパラメーター
    #LOF_limit=df_Acc_LOF["LOF"].std()*1.96 #σ=2 95%のデータは正常とする場合
    # パラメータに変更 sigma=3 #20200904修正＠野呂
    LOF_limit=df_Acc_LOF["LOF"].mean()+df_Acc_LOF["LOF"].std()*sigma

    df_Acc_outliers=df_Acc_LOF.query("LOF>@LOF_limit")
    
    plt.figure() #20200904修正＠野呂
    ...
    #df_Acc_LOF.to_csv("C:/Users/H4869/TR-BRAIN/braintest01/df_Acc_LOF.csv")#野呂確認用
    #df_Acc_outliers.to_csv("C:/Users/H4869/TR-BRAIN/braintest01/df_Acc_outliers.csv")#野呂確認用
    
    #外れ値のうち、Acc_diffが alarm_limit(デフォルト1)以上の場合だけを異常検知とする 
    df_Acc_outliers_plus=df_Acc_outliers.query("Acc_diff>{}".format(alarm_limit)) #20200904修正＠野呂 #20200909田原
    
    #外れ値のプロット
    #どのデータを外れ値として検出できたか？
    #階差によるLOFは直前の値からの差に対するLOFなので、少しづつ上がってくる場合は検知できない。あくまで急変検知。LOF_limitである程度調整可能。
    plt.figure(figsize=(20, 20))
    
    #1次階差に対し、どれが異常値か？
    plt.subplot(4,1,1)
    df_Acc_LOF["Acc_diff"].plot()
    df_Acc_outliers["Acc_diff"].plot.line(style=["r+"])

    #1次階差（正）に対し、どれが異常値か？
    plt.subplot(4,1,2)
    df_Acc_LOF["Acc_diff"].plot()
    df_Acc_outliers_plus["Acc_diff"].plot.line(style=["r+"])

    #原系列（フィルター後）に対し、どれが異常値か？
    plt.subplot(4,1,3)
    df_Acc_LOF["y"].plot()
    df_Acc_outliers["y"].plot.line(style=["r+"])
    
    #どれがトレンド急変か？
    plt.subplot(4,1,4)
    df_Acc_LOF["y"].plot()
    df_Acc_outliers_plus["y"].plot.line(style=["r+"])
    ...
    jday = datetime.datetime.utcnow() + timezoneoffset - datetime.timedelta(days=judgedays) #比較する日付
    ...
    #df_mod.to_csv("C:/Users/H4869/TR-BRAIN/df_mod.csv")#野呂：確認用
    ...
    #目的変数
    mv1_curve=df_mod["y"].rolling(mv1,min_periods=minpval).mean().tail(mv1)
    mv2_curve=df_mod["y"].rolling(mv2,min_periods=minpval).mean().tail(mv2)
    mv3_curve=df_mod["y"].rolling(mv3,min_periods=minpval).mean().tail(mv3)
    ...
    plt.subplot(4,1,1)
    plt.plot(np.arange(len(x) - len(x_mv1),len(x)), mv1_curve, color="b")
    ...
    plt.subplot(4,1,2)
    plt.plot(np.arange(len(x) - len(x_mv2),len(x)), mv2_curve, color="b")
    plt.plot(np.arange(len(x) - len(x_mv2),len(x)), y_mv2, color="y")
    plt.hlines([limit1, limit2], 0, len(x), linestyle="dashed", color="r")
    plt.title("Mooving Average {} (Total range) ".format(mv2))

    plt.subplot(4,1,3)
    plt.plot(np.arange(len(x) - len(x_mv3),len(x)), mv3_curve, color="b")
...
def main():
    ...
    if (params.guid == "braintest01"):
        battery.battery_trend(params, brainresult)
        trend_plot(params)
    else:
        try:
            battery.battery_trend(params, brainresult)
            ...
            print(e)
    brainresult["elapsed_time"] = time.time() - starttime
